...
describe StoredSubscriptionRepository do
  describe 'class' do
    ...
    it 'expects index_name' do
      expect(described_class.index_name).to(eq(Settings.get_stored_subscription_es_index_name))
    end

    it 'expects klass' do
      expect(described_class.klass).to(eq(StoredSubscription))
    end
