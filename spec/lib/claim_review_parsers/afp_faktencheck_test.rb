...
describe AFPFaktencheck do
  describe 'instance' do
    it 'has a hostname' do
      expect(described_class.new.hostname).to(eq('https://faktencheck.afp.com'))
    end
    ...
    it 'extracts a url' do
      expect(described_class.new.url_extractor(Nokogiri.parse("<a href='/blah'>wow</a>").search('a')[0])).to(eq('https://faktencheck.afp.com/blah'))
    end
