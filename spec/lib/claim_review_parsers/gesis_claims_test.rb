...
describe GESISClaims do
  describe 'instance' do
    ...
    it 'parses get_fact_ids when standard error hit' do
      RestClient::Request.stub(:execute).with(anything, anything, anything).and_raise(StandardError)
      expect(described_class.new.get_fact_ids(1)).to(eq([]))
    end

    it 'parses get_fact_ids' do
      RestClient::Request.stub(:execute).with(anything, anything, anything).and_return({ 'results' => { 'bindings' => [{ 'id' => { 'value' => '/123' } }] } }.to_json)
    ...
    it 'executes post_request_fact_id' do
      ...
      RestClient::Request.stub(:execute).with(anything, anything, anything).and_return(response)
      expect(described_class.new.post_request_fact_id("1")).to(eq(response))
    end

    it 'walks through the get_request' do
      RestClient::Request.stub(:execute).with(anything, anything, anything).and_return({}.to_json)
    ...
    it 'runs through get_claim_reviews' do
      ...
      RestClient::Request.stub(:execute).with(anything, anything, anything).and_return({ 'results' => { 'bindings' => [raw] } })
    ...
    it 'runs through get_claim_reviews in response failure' do
      ...
      RestClient::Request.stub(:execute).with(anything, anything, anything).and_return({})
      expect(described_class.new.get_claim_reviews).to(eq(nil))
    end
