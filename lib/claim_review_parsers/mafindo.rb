...
class Mafindo < ClaimReviewParser
  ...
  def parse_raw_claim_review(raw_claim_review)
    authors = @authors.select{|x| !([x["id"]]&[raw_claim_review['authors']].flatten).empty?}
    {
      ...
      claim_review_body: Nokogiri.parse("<html>"+raw_claim_review['conclusion'].to_s+"</html>").text,
      claim_review_reviewed: Nokogiri.parse("<html>"+raw_claim_review['content'].to_s+"</html>").text,
